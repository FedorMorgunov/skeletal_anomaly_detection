...
from model import SkeletonAnomalyDetector
import config
import os
...
def main():
    ...
    for epoch in range(config.NUM_EPOCHS):
        model.train()
        total_loss = 0.0
        for skeleton_sequences, label_sequences in train_loader:
            skeleton_sequences = skeleton_sequences.to(device)
            # All labels are 0 for normal sequences
            # Use the last label or entire label sequence (they are all zero)
